...
function ResultItem({
  ...
  return (
    <CaseListItem
      ...
      secondaryAction={
        <AppLink
          href={`${caseSearchPath ?? "/case-search"}/result/${
            record.case_id
          }/${searchFor}`}
        >
...
export default function CaseSearchResults({
  ...
  const searchFor = query.map((e) => decodeURIComponent(e)).join("/");
  ...
  const { data, isLoading, error, size, setSize } = useSWRInfinite<
    SearchResponse<CaseIndex> & { freeResults?: boolean }
  >(getKey, fetcher);
  const results = data;
  ...
  return (
    <Stack alignContent={"center"}>
      <CaseSearchBox
        initialSearchText={searchFor}
        caseSearchPath={caseSearchPath}
      />
      {data?.[0]?.freeResults && (
        <Alert
          variant="outlined"
          severity="warning"
          sx={{ marginX: "auto", marginY: "2em" }}
        >
          Showing results up to year 2010.{" "}
          <AppLink href="/sign-up">Sign up</AppLink> to get free unlimited
          access to all case documetns!
        </Alert>
      )}
      <FormControl>
