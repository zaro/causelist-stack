...
export default function DisplayJobs() {
  ...
  const {
    ...
    error,
    mutate,
  } = useSWR<ICorrectionJob[]>(`/api/k8s-jobs/corrections`, fetcher, {
    suspense: true,
    refreshInterval: 10000,
  });

  const runParseCorrections = () => {
    fetch(
      `/api/k8s-jobs/run-parse-corrections`,
      addAuthHeader({
        method: "POST",
      })
    ).then(async (r) => {
      ...
      ...dataColumns,
      {
        ...
        getActions: (params: GridRowParams) => [
          <GridActionsCellItem
            ...
            onClick={() => {
              window.open(
                `/api/k8s-jobs/job-log/${params.row.type}/${params.row.id}?jwt=${accessToken}`,
                "_blank"
              );
            }}
          />,
      ...
      <Grid container>
        ...
        <Grid item xs={2}>
          <div>
            ...
            >
              <MenuItem onClick={runParseCorrections}>
                Parse Corrections
              </MenuItem>
            </Menu>
          </div>
